...
type myGenerator struct {
	...
	timeoutNs	time.Duration	//超时处理 单位:纳秒
	lps		uint32//每秒荷载量
	...
	resultCh	chan *lib.CallResult	//调用结果通道,使用CallResult指针,可以用于判断nil,结构体的0值不是nil
}
...
func NewGenerator(
	...
	if resultCh == nil {
		errMsg = fmt.Sprintln("Invalid resultCh")
	}
	if errMsg != "" {
	...
	logger.Infof("Passwd. (timeoutNs=%v, lps=%d, durationNs=%v)", timeoutNs, lps, durationNs)
	err := gen.init()
	if err != nil {
		return nil, err
	}
	...
	go func() {
		defer func() {
			if p := recover(); p != nil {
				...
				result := &lib.CallResult{Id: -1, Code: lib.RESULT_CODE_FATAL_CALL, Msg: errMsg}
		...
		timer := time.AfterFunc(gen.timeoutNs, func(){
			timeout = true
			result := &lib.CallResult{Id: rawReq.Id, Req: rawReq, Code: lib.RESULT_CODE_WARNING_CALL_TIMEOUT, Msg: fmt.Sprintf("Timeout! (expected: < %v)", gen.timeoutNs)}
			gen.sendResult(result)
		})
...
func (gen *myGenerator) genLoad(throttle <-chan time.Time) {
	callCount := uint64(0)
/*	f, err := os.Create("time.log")
	if err != nil {

	}
	logger := log.New(f , "NOW : ", 0)
	*/
Loop:
	for ;; callCount++ {
		select {
