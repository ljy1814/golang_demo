...
func (comm *TcpComm) CheckResp(rawReq lib.RawReq, rawResp lib.RawResp) *lib.CallResult {
	...
	if err != nil {
		commResult.Code = lib.RESULT_CODE_FATAL_CALL
		commResult.Msg = fmt.Sprintf("Incorrectly formatted Req: %s\n", string(rawReq.Req))
		return &commResult
	}
	var sresp ServerResp
	err = json.Unmarshal(rawResp.Resp, &sresp)
//	display.Display("sresp", sresp)
	if err != nil {
		commResult.Code = lib.RESULT_CODE_ERROR_RESPONSE
	...
	if sresp.Err != nil {
		commResult.Code = lib.RESULT_CODE_ERROR_CALEE
	...
	commResult.Code = lib.RESULT_CODE_SUCCESS
