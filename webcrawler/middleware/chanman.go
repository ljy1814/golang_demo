...
const (
	CHANNEL_MANAGER_STATUS_UNINITIALIZED ChannelManagerStatus = 0 //为初始化状态
...
type ChannelManager interface{
	//初始化通道管理器，channelArgs表示通道参数,reset表示是否重新初始化通道管理器
...
//检查状态,在获取通道的时候,通道管理器应该处于已初始化状态
//如果通道管理器未处于已初始化状态则返回一个非nil的错误值
func (chanman *myChannelManager) checkStatus() error {
	...
	errMsg := fmt.Sprintf("The undesirablee status of channel manager: %s\n", statusName)
...
var chanmanSummaryTemplate = "status: %s, requestChannel: %d/%d, responseChannel: %d/%d, itemChannel: %d/%d, errorChannel: %d/%d"
