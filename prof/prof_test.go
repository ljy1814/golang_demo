...
func TestCommmonProf(t *testing.T) {
	defer func(){
	...
	fmt.Println("enter CommonProf test ...")
	makeWorkDir()
...
func makeWorkDir() {
	...
	if f == nil {
		...
		if err = os.Mkdir(absTempWorkDir, os.ModeDir | os.ModePerm); err != nil {
			panic(err)
		}
	}
	_ = f
...
func cleanWorkDir() {
	if removeTempWorkDir {
		...
		if err != nil {
			fmt.Errorf("Error: Can not remove cpu profile '%s': %s", getAbsFilePath(cpuProfilePath), err)
		}
...
func TestRuntimeProf(t *testing.T) {
	...
	for _, v := range []int{0, 1, 2} {
		for _, w := range []string{"goroutine", "threadcreate", "heap", "block"} {
			...
			fmt.Printf("Try to save %s profile to file %q ...\n", w, profileName)
